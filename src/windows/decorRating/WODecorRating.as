...
public class WODecorRating extends WindowMain{
    ...
    public function WODecorRating() {
        super();
        _windowType = WindowsManager.WO_DECOR_RATING;
        _woWidth = 680;
        _woHeight = 600;
    ...
    private function showFriend(needShow:Boolean = false):void {
        var cafeItem:WODecorRatingItem;
        for (var i:int =0; i < g.managerFarmStandCoinsMaxRating.arrCoinsMaxRatingFriend.length; i ++) {
            cafeItem = new WODecorRatingItem(i+1, g.managerFarmStandCoinsMaxRating.arrCoinsMaxRatingFriend[i].userId, g.managerFarmStandCoinsMaxRating.arrCoinsMaxRatingFriend[i].count, g.managerFarmStandCoinsMaxRating.arrCoinsMaxRatingFriend[i].userSocialId);
    ...
    private function showAll(needShow:Boolean = false):void {
        ...
        for (var i:int =0; i < g.managerFarmStandCoinsMaxRating.arrCoinsMaxRating.length; i ++) {
            cafeItem = new WODecorRatingItem(i+1,g.managerFarmStandCoinsMaxRating.arrCoinsMaxRating[i].userId, g.managerFarmStandCoinsMaxRating.arrCoinsMaxRating[i].count, g.managerFarmStandCoinsMaxRating.arrCoinsMaxRating[i].userSocialId);
            _scrollSprite.addNewCell(cafeItem.source);
            _arrItem.push(cafeItem);
        }
        checkSocialInfoForArray();
    ...
    private function onGettingInfo(e:SocialNetworkEvent):void {
        ...
        for (var i:int = 0; i < _arrItem.length; i++) {
            (_arrItem[i] as WODecorRatingItem).updateAvatar();
        }
    }
}
}
